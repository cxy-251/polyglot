"""537｜学习 http.server 的请求解析、响应、静态文件、CGI 与服务器组合。
...
# 538｜HTTP 请求错误边界：语法、版本、请求行/头长度与未实现方法。
#
...
# 539｜HTTP/1.1 持久连接、Connection 指令与 Expect: 100-continue。
#
...
# 540｜响应状态/头缓冲、显式结束、版本字符串、日期与连接状态副作用。
#
...
# 541｜send_error 的完整响应、HTML 转义、HEAD/无正文状态与未知状态码。
#
...
# 542｜SimpleHTTPRequestHandler 的 URL→本地路径转换、MIME 推断与目录参数。
#
...
# 543｜SimpleHTTPRequestHandler 的 GET/HEAD 文件工作流与二进制复制。
#
...
# 545｜静态文件的 If-Modified-Since、If-None-Match 优先规则与 404 边界。
#
...
# 546｜CGIHTTPRequestHandler 的目录式识别、路径折叠、POST 限制与 Python 后缀。
#
...
# 547｜HTTPServer 类组合、绑定元数据、日志控制字符清洗与符号链接风险。
#
