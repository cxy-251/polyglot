"""565｜SimpleXMLRPCDispatcher 的函数注册、线路分派、覆盖回调与 Fault 转换。
...
# 566｜register_instance 的直接/自定义分派、点分名称开关、优先级与私有属性保护。
#
...
# 567｜服务端 introspection API、帮助/签名回退与 system.multicall 的逐项隔离。
#
...
# 568｜SimpleXMLRPCServer 构造参数传递、延迟绑定，以及请求路径/Accept-Encoding 解析。
#
...
# 569｜SimpleXMLRPCRequestHandler.do_POST 的 HTTP 200 调用/Fault 与路径 404。
#
...
# 570｜XML-RPC HTTP 处理器的 gzip 请求解码、响应协商与内容编码错误。
#
...
# 571｜MultiPathXMLRPCServer 的路径→Dispatcher 映射、虚拟服务隔离与缺失路径 Fault。
#
...
# 572｜CGIXMLRPCRequestHandler 的显式请求、stdin 长度读取、stdout 头/正文与 GET 错误。
#
...
# 573｜XMLRPCDocGenerator 的标题/说明/方法文档，以及 HTTP/CGI 文档服务器组合。
#
