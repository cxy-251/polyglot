"""559｜xmlrpc.client.dumps/loads 的请求包、核心类型映射、自定义对象与格式限制。
...
# 560｜XML-RPC 的 nil 扩展、base64、日期时间、use_builtin_types 与 Decimal 解码。
#
...
# 561｜DateTime/Binary 包装器的构造、编码钩子、解码、比较与原始数据访问。
#
...
# 562｜XML-RPC Fault 与 HTTP ProtocolError 的分层、字段、字符串和故障包往返。
#
...
# 563｜ServerProxy 的动态点分方法、单值解包、自定义 Transport、认证与上下文关闭。
#
...
# 564｜MultiCall 的延迟收集、点分方法、system.multicall 批量包与逐项 Fault。
#
