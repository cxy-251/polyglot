...
# ensure_ascii、indent、separators、sort_keys 与对象 key 规则。
#
...
# default 扩展点、JSONEncoder 分块输出与循环引用检查。
#
...
# JSONDecoder.raw_decode、strict 控制字符与结构化错误位置。
#
...
# NaN/Infinity、重复名称与顶层 scalar 的兼容性边界。
#
...
# Unicode、UTF bytes 自动探测、BOM 差异与不可信输入限额。
#
...
# JSON 不是 framed protocol，以及 JSON Lines 工作流。
#
...
# python -m json.tool 的验证、格式化与 JSON Lines 模式。
#
