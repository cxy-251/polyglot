"""091｜``multiprocessing.Pool`` apply/map families、initializer 与 worker recycling。
...
# Pool async results、timeout、callbacks 与 remote exception transport。
#
...
# ``multiprocessing.Manager`` server process、proxy semantics 与 nested state。
#
...
# ``BaseManager.register`` custom referent、exposed methods 与 client authentication。
#
...
# ``multiprocessing.connection`` Listener/Client 与 HMAC challenge helpers。
#
...
# multiprocessing misc introspection、logger 与 thread-backed dummy Pool。
#
...
# ``SharedMemory`` create/attach、memoryview alias 与 close/unlink lifecycle。
#
...
# ``ShareableList`` supported scalars、fixed layout、capacity 与 name attachment。
#
...
# ``SharedMemoryManager`` ownership、managed blocks/lists 与 automatic unlink。
#
