"""092｜``concurrent.futures.Future`` pending/running/cancelled/finished state machine。
...
# ``wait`` completion policies、duplicate elimination 与 ``as_completed`` timeout。
#
...
# ``ThreadPoolExecutor`` submit/map、initializer、thread naming 与 idle reuse。
#
...
# ThreadPool cancellation、``shutdown(cancel_futures)`` 与 BrokenThreadPool。
#
...
# ``ProcessPoolExecutor`` mp_context、pickling boundary、map 与 remote exception。
#
...
# ProcessPool initializer failure、abrupt worker exit 与 BrokenProcessPool。
#
