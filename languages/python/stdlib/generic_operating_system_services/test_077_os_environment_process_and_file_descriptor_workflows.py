"""077｜``os.environ``/``environb``、getenv、putenv trap 与 executable PATH。
...
# ``os`` process identity、cwd、fchdir、umask 与 Unix identity queries。
#
...
# ``os.open/read/write/lseek``、fdopen、fstat、truncate 与 buffering 边界。
#
...
# ``os.pipe``、dup/dup2、shared offset、blocking mode 与 inheritable flag。
#
...
# ``os.pread/pwrite``、readv/writev、preadv/pwritev 与 kernel-side copy。
#
...
# ``os`` 的预分配、访问建议、record lock 与 ``sendfile``。
#
