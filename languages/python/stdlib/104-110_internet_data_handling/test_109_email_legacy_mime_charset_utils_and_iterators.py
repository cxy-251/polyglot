"""109｜legacy MIMEBase、MIMEMultipart 与 MIMENonMultipart。
...
# MIMEApplication、MIMEImage、MIMEAudio、MIMEMessage 等便捷类。
#
...
# MIMEText 的 charset 推断，以及已有 CTE header 带来的旧 API 陷阱。
#
...
# email.encoders 的 base64、quoted-printable、7bit/8bit 与 noop。
#
...
# legacy Header 的多字符集片段、折行、解码与重建。
#
...
# Charset 的规范化、编码策略与按行 header 编码。
#
...
# email.charset 的全局 alias、字符集策略与 codec 注册表。
#
...
# compat32 Message 的 Unix-From、list payload 与 set_charset。
#
...
# email.utils 的 Message-ID、地址拆装与引号转义。
#
...
# RFC 2231 参数的编码、分段解码与最终 Unicode 折叠。
#
