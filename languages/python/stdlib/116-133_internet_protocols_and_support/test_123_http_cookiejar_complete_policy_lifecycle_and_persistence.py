"""123｜学习 CookieJar 的提取、策略、生命周期、Cookie 对象与文件持久化。
...
# CookieJar 的身份键、替换、精确清除、会话清除与过期清理。
#
...
# CookieJar.make_cookies 的协议版本、RFC 2109 降级与受策略/无策略写入。
#
...
# DefaultCookiePolicy 的阻止/允许域名列表、点前缀语义与动态替换。
#
...
# CookiePolicy 的域回送严格度、路径边界、Secure 与第三方不可验证请求。
#
...
# Cookie 值对象的标准字段、端口/域标记、扩展属性与显式过期判断。
#
...
# MozillaCookieJar 的 Netscape 文件格式、持久/会话过滤与 PathLike 文件名。
#
...
# LWPCookieJar 的 Set-Cookie3 文本、load 合并、revert 替换与原子失败。
#
