"""112｜ElementTree 从字符串、片段列表、文件解析，以及 XMLID/dump/iselement。
...
# Element/SubElement、Comment、ProcessingInstruction、QName 与 text/tail。
#
...
# Element 的属性映射、子节点序列、浅复制、itertext 与 clear。
#
...
# ElementTree 的 find/findall/iterfind/findtext、深度遍历与 namespace。
#
...
# ElementTree 支持的 XPath 属性/文本谓词、位置函数与边界。
#
...
# ElementTree 节点移除、根替换与 ``indent()`` 的可变语义。
#
...
# ``tostring``、``tostringlist`` 与 ``ElementTree.write`` 的输出契约。
#
...
# ElementTree 序列化时的前缀注册、默认命名空间与 QName 值。
#
...
# ``XMLPullParser`` 的 feed/read_events 队列与非阻塞解析模式。
#
...
# ``XMLParser`` target 协议与 ``TreeBuilder`` 的注释、PI 插入选项。
#
...
# ElementTree 的 C14N 2.0 规范化输出与 ``C14NWriterTarget``。
#
...
# ``ElementInclude`` 的 XML/text 替换、自定义 loader 与递归限制。
#
...
# ``ParseError`` 诊断信息与 XML 不可信输入的安全边界。
#
