...
A.case("local lock and install record the path dependency and Bundler version") do
...
A.case("frozen bundle exec uses an existing lock while frozen install refuses to create one") do
  application = P.write_bundler_app("bundle-frozen-exec")
  environment = P.bundler_environment(application)
  %w[lock install].each do |command|
    stdout, stderr, status = P.run_tool(
      "bundle",
