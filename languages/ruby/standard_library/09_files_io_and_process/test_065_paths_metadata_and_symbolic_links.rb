...
A.case("Pathname composes lexical cleanup with explicit file-system operations") do
...
A.case("lstat observes a symbolic link while stat and realpath follow its target") do
  root = H.temporary_path("link-tree")
  FileUtils.mkdir_p(File.join(root, "nested"))
  path = Pathname(root).join("nested", "value.txt")
  path.write("ruby")
