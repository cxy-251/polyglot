...
九门 active language 集合已经冻结，不再增加第十门语言。当前正在按 Ruby、Lua、R、
Julia、Rust、Go、Node.js、C++、Python 的顺序进行文件级语义审计；审计前的文件数和
测试结果只是执行快照，不表示内容已经完成审查。
...
这些主题和审计前每门语言 73 个横向入口都是待审输入，不是必须维持的指标。审计会检查
共同问题是否明确、各语言是否真的回答同一问题、名称是否过宽、是否强行等价以及关联课程
是否实际支撑断言；无效主题可以合并、重命名或删除。
...
固定安装路径、容器命令、仓库 fixture 布局和环境变量清理属于 harness。审计会逐语言
移动现有混合内容，避免一次跨九门语言的模板化重排。
...
49 个主题构成当前审计输入。修改优先补强、合并、纠错或删除现有主题；增加主题或章节前
先确认：
